...
namespace SpaceInvaders.Animations
{
    public class CellAnimator : SpriteAnimator
    {
        ...
        private readonly int r_FirstCellIdx;

        public TimeSpan CellTime { get => m_CellTime; set => m_CellTime = value; }
            ...
            : base("CellAnimation", i_AnimationLength)
        {
            ...
            this.r_FirstCellIdx = 0;

            m_Loop = i_AnimationLength == TimeSpan.Zero;
        }
        public CellAnimator(TimeSpan i_CellTime,int i_StartCellIdx, int i_NumOfCells, TimeSpan i_AnimationLength)
           : base("CellAnimation", i_AnimationLength)
        {
            this.m_CurrCellIdx = i_StartCellIdx;
            this.r_FirstCellIdx = m_CurrCellIdx;
            this.m_CellTime = i_CellTime;
            this.m_TimeLeftForCell = i_CellTime;
            this.r_NumOfCells = i_NumOfCells;
        ...
        {
            m_CurrCellIdx++;
            if (m_CurrCellIdx >= r_NumOfCells + r_FirstCellIdx)
            {
                if (m_Loop)
                {
                    m_CurrCellIdx = r_FirstCellIdx;
                }
                else
                {
